  ...
  try {
    ...
    if (!apiKey || !secretKey) {
      console.error("[API] Missing Stripo credentials:", {
        ...
        allEnvKeys: Object.keys(process.env).filter(key => key.includes('STRIPO')),
      });
      return NextResponse.json(
        { 
          ...
          availableEnvVars: process.env.NODE_ENV === 'development' 
    ...
    if (apiKey && apiKey.startsWith('eyJ')) {
      ...
      if (decoded) {
        console.warn("[API] STRIPO_PLUGIN_ID appears to be a JWT token. Attempting to extract pluginId...");
        ...
        if ('pluginId' in decoded && typeof decoded.pluginId === 'string') {
          ...
          console.error("[API] ✗ JWT decoded but no pluginId/id found. JWT payload keys:", Object.keys(decoded));
          console.error("[API] WARNING: Using JWT token as pluginId - this will likely fail. Please set STRIPO_PLUGIN_ID to the actual plugin ID string.");
        }
      }
    }
    ...
    if (pluginIdToUse === apiKey && apiKey.length > 50) {
      try {
        ...
        if (typeof parsed === 'object' && parsed !== null) {
          console.warn("[API] STRIPO_PLUGIN_ID appears to be JSON. Attempting to extract pluginId...");
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log("[API] Stripo auth request:", {
        ...
        pluginIdPreview: pluginIdToUse ? `${pluginIdToUse.substring(0, 10)}...${pluginIdToUse.substring(pluginIdToUse.length - 6)}` : undefined,
        userId,
        role: "user",
      });
    }
    ...
    if (!response.ok) {
      ...
      console.error("[API] Stripo auth error - full details:", {
        ...
        apiKeyPreview: apiKey ? `${apiKey.substring(0, 20)}...${apiKey.substring(apiKey.length - 10)}` : undefined,
        secretKeyPreview: secretKey ? `${secretKey.substring(0, 4)}...${secretKey.substring(secretKey.length - 4)}` : undefined,
        url: STRIPO_AUTH_URL,
        requestBody: {
      ...
      if (response.status === 404) {
        errorMessage = "Stripo authentication endpoint not found. Please verify your STRIPO_PLUGIN_ID and STRIPO_SECRET_KEY are correct.";
      } else if (response.status === 401) {
        errorMessage = "Invalid Stripo credentials. Please check your STRIPO_PLUGIN_ID and STRIPO_SECRET_KEY.";
      } else if (response.status === 400) {
        errorMessage = error.message || "Invalid request to Stripo API. Please check your credentials format.";
      }
